# Проект ServicePrintCheckPHP - Архитектура

Этот проект представляет собой PHP-сервис для взаимодействия с кассовыми аппаратами (ККТ) и другими внешними устройствами (банковские терминалы, весы) через COM-объекты на операционной системе Windows. Архитектура приложения построена на принципах Model-View-Presenter (MVP) с акцентом на чистое разделение ответственности и инъекцию зависимостей.
...
*   **Слой Модели (Model) (`CheckService.php`, `kktutils.php`, `models.php`, `validators.php`, `settings.php`):** Содержит основную бизнес-логику, взаимодействие с драйверами и внешними устройствами, структуры данных, логику валидации и управления настройками.
