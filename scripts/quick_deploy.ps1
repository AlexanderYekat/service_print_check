...
try {
    ...
    if (Get-Command composer -ErrorAction SilentlyContinue) {
        ...
        Write-Host "⚠️  Composer не найден, пропускаем установку зависимостей" -ForegroundColor Yellow
    }
    ...
    Write-Host "⚙️  Проверка настроек..." -ForegroundColor Yellow
    ...
    if (Test-Path $testFile) {
        try {
            ...
            Write-Host "⚠️  Некоторые тесты провалились, но продолжаем" -ForegroundColor Yellow
        }
    } else {
        Write-Host "ℹ️  Тесты не найдены" -ForegroundColor Gray
    }
    ...
    if (Test-Path "public/index.php") {
        ...
        Write-Host "ℹ️  Файл index.php не найден" -ForegroundColor Gray
    }
