Перем ТекущийИдентификаторЗадачи, МаксимальноеКоличествоПопыток, ТекущаяПопытка;
...
Процедура КнопкаПроверитьМаркуНаККТАсинхНажатие(Элемент)
	...
	ТекущийИдентификаторЗадачи = ПечатьЧекаККТАсинх.ПроверитьМаркуНаККТ(марка);
...
Процедура ПроверитьСтатусЗадачиМаркировки() Экспорт
    ...
    // Запрашиваем статус задачи (быстрый таймаут)
    Попытка
        Соединение = Новый HTTPСоединение("localhost", 8000, , , , 5); // 5 секунд вместо 30
